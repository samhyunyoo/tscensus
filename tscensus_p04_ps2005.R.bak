...
tscensus2005 <- readRDS("data/tscensus2005.rds")
ts2005 <- tscensus2005 |> 
  mutate(
    ...
    educ = case_when(
      ...
      edua == 4 & edub == 1 ~ "HS", 
      (edua == 5 & edub !=1) ~ "SomeCol", 
      (edua == 5 & edub ==1) | edua %in% c(6) ~ "BA+"
    ), 
    educ = factor(educ, levels = c("<HS", "HS", "SomeCol", "BA+")), 
  
    mar = as.numeric(V37), 
    agefm = as.numeric(V38), 
    agefmgr = case_when(agefm <  20 ~ "<20", 
                        agefm >= 20 & agefm < 25 ~ "20-24", 
                        agefm >= 25 & agefm < 30 ~ "25-29", 
                        agefm >= 30 & agefm < 35 ~ "30-34", 
                        agefm >= 35 & agefm < 40 ~ "35-39", 
                        agefm >= 40  ~ "40+", 
                        TRUE ~ NA_character_
    ),
    
    ceb_boy = as.numeric(V39), 
    ceb_girl = as.numeric(V40), 
    ceb = rowSums(across(c(ceb_boy, ceb_girl)), na.rm = TRUE), 
    
    # expkid = as.numeric(V78), 
    expnkids = as.numeric(V47), 
...
ggplot(pop2005, aes(x = agegr, y = pop_signed, fill = res_admin)) +
  geom_col(width = 0.9) +
  coord_flip() +
...
ggsave("graphs/prd2005_res_admin.png", width = 10, height = 5)

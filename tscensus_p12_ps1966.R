...
tscensus1966 <- readRDS("data/tscensus1966.rds")
ts1966 <- tscensus1966 |> 
  mutate(
    ...
    educ = case_when(
      edua < 4 | (edua == 4 & edub != 2) | edua ==7 ~ "<HS", 
      edua == 4 & edub == 2 ~ "HS", 
      edua == 5 | (edua == 6 & edub !=2) ~ "SomeCol", 
      (edua == 6 & edub ==2) ~ "BA+", 
      TRUE ~ NA_character_
    ), 
    educ = factor(educ, levels = c("<HS", "HS", "SomeCol", "BA+")), 
    
    mar1966 = as.numeric(V12), 
    mar = ifelse(mar1966 == 4, 1, 
                 ifelse(is.na(mar1966), NA, mar1966+1)),
    ...
    ceb = ifelse(V15 == "99", NA, as.numeric(V15)),
...
ggplot(pop1966, aes(x = agegr, y = pop_signed, fill = res_admin)) +
  geom_col(width = 0.9) +
  coord_flip() +
...
ggsave("graphs/prd1966_res_admin.png", width = 10, height = 5)

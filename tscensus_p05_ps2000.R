...
tscensus2000 <- readRDS("data/tscensus2000.rds")
ts2000 <- tscensus2000 |> 
  mutate(
    ...
    ceb_boy = ifelse(V43 == "99", NA, as.numeric(V43)), 
    ceb_girl = ifelse(V42 == "99", NA, as.numeric(V42)), 
    # ceb = rowSums(across(c(ceb_boy, ceb_girl)), na.rm = TRUE), 
    ceb = ifelse(V44 == "99", NA, as.numeric(V44)),
...
ggplot(pop2000, aes(x = agegr, y = pop_signed, fill = res_admin)) +
  geom_col(width = 0.9) +
  coord_flip() +

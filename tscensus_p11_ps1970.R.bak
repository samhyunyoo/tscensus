...
tscensus1970 <- readRDS("data/tscensus1970.rds")
ts1970 <- tscensus1970 |> 
  mutate(
    ...
    educ = case_when(
      edua < 3 | (edua == 3 & edub != 2) | edub == 4 ~ "<HS", 
      edua == 3 & edub == 2 ~ "HS", 
      edua == 4 | (edua == 5 & edub !=2) ~ "SomeCol", 
      (edua == 5 & edub ==2)  ~ "BA+", 
      TRUE ~ NA_character_
    ), 
    educ = factor(educ, levels = c("<HS", "HS", "SomeCol", "BA+")), 
 
    mar1970 = as.numeric(V14), 
    mar = ifelse(mar1970 == 4, 1, 
                 ifelse(is.na(mar1970), NA, mar1970+1)),   
...
ggplot(pop1970, aes(x = agegr, y = pop_signed, fill = res_admin)) +
  geom_col(width = 0.9) +
  coord_flip() +
...
ggsave("graphs/prd1970_res_admin.png", width = 10, height = 5)

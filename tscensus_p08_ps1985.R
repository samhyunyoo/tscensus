...
tscensus1985 <- readRDS("data/tscensus1985.rds")
ts1985 <- tscensus1985 |> 
  mutate(
    ...
    educ = case_when(
      edua < 3 | (edua == 3 & edub != 1) | edua == 6 ~ "<HS", 
      edua == 3 & edub == 1 ~ "HS", 
      edua == 4 | (edua == 5 & edub !=1) ~ "SomeCol", 
      (edua == 5 & edub ==1) ~ "BA+"
    ), 
    ...
    ceb = ifelse(V33 == "99", NA, as.numeric(V33)),
...
ggplot(pop1985, aes(x = agegr, y = pop_signed, fill = res_admin)) +
  geom_col(width = 0.9) +
  coord_flip() +
...
ggsave("graphs/prd1985_res_admin.png", width = 10, height = 5)
